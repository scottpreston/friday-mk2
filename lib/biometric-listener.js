...
function handleData() {
    port.on('data', function (data) {
        ...
        var rightArm = vals[0];
        var leftArm = vals[1];
        if (rightArm > 4 && right == "down") {
            sendThrottledRequest(RIGHT_ARM_UP);
            right = "up";
        }
        if (rightArm < 4 && right == "up") {
            sendThrottledRequest(RIGHT_ARM_DOWN);
            right = "down";
        }
        if (leftArm > 4 && left == "down") {
            sendThrottledRequest(LEFT_ARM_UP);
            left = "up";
        }
        if (leftArm < 4 && left == "up") {
            sendThrottledRequest(LEFT_ARM_DOWN);
            left = "down";
        }
        //if (rightArm >= 2 && rightArm <= 3) sendRequest(RIGHT_ARM_STOP);
        //if (leftArm >= 2 && rightArm <= 3) sendRequest(LEFT_ARM_STOP);
    });
...
function sendThrottledRequest(url) {
